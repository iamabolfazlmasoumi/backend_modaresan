...
const HRController = {
    assignHR: async function (req, res, next) {
        try {
            ...
            if (!role.permissions.includes(access._id)) {
                return next(new OperationalError(NOT_AUTHORIZED_ERROR, "assignHR"));
            }
            ...
            let depsCount = await Department.find({
                isDeleted: false
            }).count();
    ...
    unassignHR: async function (req, res, next) {
        try {
            ...
            if (!role) return next(new OperationalError(NOT_FOUND_ERROR, "role"));
            if (!role.permissions.includes(access._id)) {
                return next(new OperationalError(NOT_AUTHORIZED_ERROR, "unassignHR"));
            }
    ...
    HRList: async function (req, res, next) {
        try {
            ...
            if (!role) return next(new OperationalError(NOT_FOUND_ERROR, "role"));
            if (!role.permissions.includes(access._id)) {
                return next(new OperationalError(NOT_AUTHORIZED_ERROR, "HRList"));
            }
